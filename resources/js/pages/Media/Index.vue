...
import {
    useMediaFilters,
    type MediaFilters as MediaFiltersState,
} from '@/composables/useMediaFilters';
...
const { filters, normalizedFilters, resetFilters } = useMediaFilters(
    props.filters,
);

const selectedIds = ref<number[]>([]);
const viewMode = ref<'gallery' | 'list'>(
    localStorage.getItem('media.viewMode') === 'list' ? 'list' : 'gallery',
);
...
const allSelected = computed(() => {
    return (
        props.media.data.length > 0 &&
        selectedIds.value.length === props.media.data.length
    );
});
...
const handleBulkDelete = async () => {
    ...
    try {
        await axios.post('/media/bulk-delete', {
...
const handleCreateTag = async (name: string) => {
    ...
    try {
        ...
        bulkTagIds.value = Array.from(
            new Set([...bulkTagIds.value, newTag.id]),
        );
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 p-6">
            <div class="flex flex-wrap items-center gap-4">
                <div>
                    <h1
                        class="text-3xl font-semibold text-gray-900 dark:text-gray-50"
                    >
                        Media Gallery
                    </h1>
                    <p class="text-sm text-gray-500">
                        Upload images and PDFs, organize with tags, and manage
                        them in bulk.
                    </p>
                </div>
                <div class="ml-auto flex items-center gap-3">
                    ...
                    <div
                        class="flex rounded-full border border-gray-200 bg-white p-1 shadow-sm dark:border-gray-800 dark:bg-gray-900"
                    >
                        <Button
                            size="icon"
                            variant="ghost"
            ...
            <MediaFilters
                ...
                :tags="availableTags"
                @clear="clearFilters"
                @change="applyFilterChanges"
            />

            <div
                v-if="media.data.length === 0"
                class="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 p-10 text-center dark:border-gray-800"
            >
                <p
                ...
                <p class="text-sm text-gray-500">
                    Upload images or PDFs to see them here.
                </p>
                <Button class="mt-4" @click="showUploader = true"
                    >Upload Media</Button
                >
            </div>
            ...
            <div v-else>
                ...
                <div
                    v-else
                    class="overflow-hidden rounded-xl border border-gray-200 shadow-sm dark:border-gray-800"
                >
                    <table
                    ...
                    >
                        <thead class="bg-gray-50 dark:bg-gray-900/60">
                            <tr>
                                ...
                                <th
                                    class="cursor-pointer px-4 py-3 text-left text-xs font-semibold tracking-wide text-gray-500 uppercase hover:text-gray-700 dark:hover:text-gray-200"
    ...
    <Dialog v-model:open="tagDialogOpen">
        <DialogContent class="max-w-lg">
            <DialogHeader>
                ...
                <DialogDescription
                    >Select which tags to
                    {{ bulkTagMode === 'add' ? 'add' : 'remove' }} for the
                    selected media.</DialogDescription
                >
            </DialogHeader>
            <MediaTagInput
                v-model="bulkTagIds"
            ...
            />
            <DialogFooter>
                <Button variant="ghost" @click="tagDialogOpen = false"
                    >Cancel</Button
                >
