...
interface ContentDerivative {
    ...
    content_piece: ContentPiece | null;
}
...
interface ActivityLog {
    ...
    user: User | null;
    content_derivative: ContentDerivative | null;
    is_comment: boolean;
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Derivative Activities', href: '/derivative-activities' },
];
...
<template>
    <Head title="Derivative Activities" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="mb-6">
                <h1 class="text-2xl font-semibold">Derivative Activities</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Track comments, generation events, and status changes for your content derivatives.
                </p>
            </div>

            <!-- Filters -->
            <div
                class="mb-6 rounded-lg border bg-white p-4 dark:border-gray-700 dark:bg-gray-800"
            >
                ...
                >
                    ...
                    >
                        <tr v-for="log in logs.data" :key="log.id">
                            ...
                            >
                                <div class="flex items-center gap-2">
                                    <MessageCircle v-if="log.is_comment" class="h-4 w-4 text-blue-500" />
                            ...
                            <td class="px-6 py-4 text-sm whitespace-nowrap">
                                <div v-if="log.content_derivative">
                                    <Link
                                        v-if="log.content_derivative.content_piece"
                                        :href="`/content-pieces/${log.content_derivative.content_piece.id}/edit?tab=derivatives${log.content_derivative.channel ? `&channel=${log.content_derivative.channel.id}` : ''}`"
                                        class="text-primary hover:underline"
                                    >
                                        {{ log.content_derivative.content_piece.internal_name }}
                                    </Link>
                                    <div v-if="log.content_derivative.channel" class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ log.content_derivative.channel.name }}
                                    </div>
                                </div>
                                <span v-else class="text-gray-400">-</span>
                            </td>
                            <td
                                class="max-w-md px-6 py-4 text-sm text-gray-500 dark:text-gray-400"
                            >
                                <span :class="{ 'italic': log.is_comment }">
                                    {{ log.description || '-' }}
                                </span>
                            </td>
                            <td
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div v-if="log.content_derivative">
                                                ...
                                                <div class="mt-1 text-sm">
                                                    ...
                                                    <span v-if="log.content_derivative.channel">
                                                        ({{ log.content_derivative.channel.name }})
                                                    </span>
                                                </div>
                                            </div>
                        ...
                        <tr v-if="logs.data.length === 0">
                            ...
                            >
                                No derivative activities found for the selected filters.
                            </td>
                        </tr>
                    </tbody>
                </table>
