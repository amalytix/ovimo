    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Monitor your OpenAI API token consumption.</p>
            </div>
            ...
            <div class="mb-8 grid grid-cols-1 gap-4 md:grid-cols-4">
                <div class="rounded-lg border bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div class="mt-2 text-3xl font-bold">{{ formatNumber(totalStats.total_tokens) }}</div>
                </div>
                <div class="rounded-lg border bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Requests</div>
                    <div class="mt-2 text-3xl font-bold">{{ formatNumber(totalStats.total_requests) }}</div>
                </div>
                <div class="rounded-lg border bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Input Tokens</div>
                    <div class="mt-2 text-3xl font-bold">{{ formatNumber(totalStats.total_input) }}</div>
                </div>
                <div class="rounded-lg border bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Output Tokens</div>
                    <div class="mt-2 text-3xl font-bold">{{ formatNumber(totalStats.total_output) }}</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                <!-- Usage by Operation -->
                <div class="rounded-lg border bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    <h2 class="mb-4 text-lg font-semibold">Usage by Operation</h2>
                    <div v-if="byOperation.length > 0" class="space-y-4">
                        <div v-for="item in byOperation" :key="item.operation" class="flex items-center justify-between">
                            <div>
                                <div class="font-medium">{{ formatOperation(item.operation) }}</div>
                                <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatNumber(item.requests) }} requests</div>
                            </div>
                            <div class="text-right">
                                <div class="font-semibold">{{ formatNumber(item.tokens) }}</div>
                ...
                <div class="rounded-lg border bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div v-if="byModel.length > 0" class="space-y-4">
                        <div v-for="item in byModel" :key="item.model" class="flex items-center justify-between">
                            <div>
            ...
            <div class="mt-8 rounded-lg border bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                <h2 class="mb-4 text-lg font-semibold">Daily Usage (Last 30 Days)</h2>
                <div v-if="dailyUsage.length > 0" class="overflow-x-auto">
                    <div class="flex min-w-full items-end gap-1" style="height: 200px">
                        <div
                ...
                <div v-else class="text-sm text-gray-500 dark:text-gray-400">No daily usage data yet.</div>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                            ...
                            <tr v-if="recentLogs.length === 0">
                                <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                                    No usage logs yet. Generate some content to see your token usage.
                                </td>
                            </tr>
                        </tbody>
                    </table>
