...
const sortBy = (column: string) => {
    if (localFilters.value.sort_by === column) {
        ...
        localFilters.value.sort_direction =
            localFilters.value.sort_direction === 'asc' ? 'desc' : 'asc';
    } else {
        // New column, default to desc for found_at and relevancy, asc for others
        localFilters.value.sort_by = column;
        localFilters.value.sort_direction =
            column === 'found_at' || column === 'relevancy' ? 'desc' : 'asc';
    }
    applyFilters();
};
...
const bulkDelete = () => {
    ...
    if (
        !confirm(
            `Are you sure you want to delete ${selectedPosts.value.length} post(s)? This action cannot be undone.`,
        )
    )
        return;
    router.post(
...
const hideNotRelevant = () => {
    if (
        !confirm(
            'Are you sure you want to hide all posts with status "Not Relevant"? This action cannot be undone.',
        )
    )
        return;
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            ...
            <!-- Filters -->
            <div
                class="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900"
            >
                ...
                <div class="w-24">
                    <Input
                        id="min_relevancy"
                        v-model.number="localFilters.min_relevancy"
                        type="number"
                        min="0"
                        max="100"
                        placeholder="Min %"
                    ...
                    <Select v-model="localFilters.status">
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value="NOT_RELEVANT"
                                >Not Relevant</SelectItem
                            >
                            <SelectItem value="CREATE_CONTENT"
                                >Create Content</SelectItem
                            >
                        </SelectContent>
                    </Select>
                </div>
                ...
                <div class="flex items-center gap-2">
                    <Checkbox
                        id="show_hidden"
                        :model-value="localFilters.show_hidden"
                        @update:model-value="localFilters.show_hidden = $event"
                    />
                    <Label
                        for="show_hidden"
                ...
                >
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th
                                class="px-6 text-left text-xs font-medium tracking-wider text-gray-500 uppercase dark:text-gray-400"
                            >
                                Summary
                            </th>
                            <th
                    ...
                    >
                        <tr
                            ...
                            :key="post.id"
                            :class="{ 'opacity-80': post.is_hidden }"
                        >
                            <td class="px-4 py-4">
                                <Checkbox
                                    :model-value="
                            ...
                            <td class="px-6 py-4 text-sm whitespace-nowrap">
                                <span
                                    ...
                                    :class="{
                                        'text-green-600':
                                            post.relevancy_score >= 70,
                                        'text-yellow-600':
                                            post.relevancy_score >= 40 &&
                                            post.relevancy_score < 70,
                                        'text-red-600':
                                            post.relevancy_score < 40,
                                    }"
                            ...
                            >
                                ...
                                >
                                    <button
                                        ...
                                        @click="
                                            createContentPieceFromPost(post.id)
                                        "
                                    >
                                        <WandSparkles class="h-4 w-4" />
                                        <span class="sr-only"
                                            >Create content piece</span
                                        >
                                    </button>
                                    <button
                                        type="button"
