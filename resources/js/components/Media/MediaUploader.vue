...
const isUploading = computed(() =>
    uploads.some((item) =>
        ['uploading', 'storing', 'pending'].includes(item.status),
    ),
);
    ...
    <Dialog :open="open" @update:open="emit('update:open', $event)">
        <DialogContent class="max-w-3xl">
            <DialogHeader>
                ...
                <DialogDescription
                    >Images and PDFs up to 50MB each.</DialogDescription
                >
            </DialogHeader>
            ...
            >
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Supported: JPEG, PNG, GIF, WebP, SVG, PDF
                </p>
                <input
                    ref="fileInput"
                    type="file"
                    ...
                    @change="handleFiles($event.target?.files || null)"
                />
            </div>
            ...
            <div v-if="uploads.length > 0" class="space-y-3">
                ...
                >
                    <div
                        class="flex h-12 w-12 items-center justify-center overflow-hidden rounded-md bg-gray-100 dark:bg-gray-800"
                    >
                        <img
                            v-if="upload.preview"
                            :src="upload.preview"
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            <span v-if="upload.status === 'complete'"
                                >Upload complete</span
                            >
                            <span v-else-if="upload.status === 'storing'"
                                >Saving to library…</span
                            >
                            <span v-else-if="upload.status === 'uploading'"
                                >Uploading…</span
                            >
                            <span
