...
test('authenticated users can toggle post hidden status', function () {
    ...
    $this->actingAs($user)
        ->patch("/news/{$post->id}/toggle-hidden")
        ->assertRedirect();
...
test('users cannot toggle hidden status for other teams posts', function () {
    ...
    $this->actingAs($user)
        ->patch("/news/{$otherPost->id}/toggle-hidden")
        ->assertForbidden();
...
test('authenticated users can update post status', function () {
    ...
    $this->actingAs($user)
        ->patch("/news/{$post->id}/status", ['status' => 'CREATE_CONTENT'])
        ->assertRedirect();
...
test('post status update validates status value', function () {
    ...
    $this->actingAs($user)
        ->patch("/news/{$post->id}/status", ['status' => 'INVALID'])
...
test('users cannot update status for other teams posts', function () {
    ...
    $this->actingAs($user)
        ->patch("/news/{$otherPost->id}/status", ['status' => 'CREATE_CONTENT'])
        ->assertForbidden();
...
test('posts index filters by source', function () {
    ...
    $response = $this->actingAs($user)->get("/news?source_id={$source1->id}");
